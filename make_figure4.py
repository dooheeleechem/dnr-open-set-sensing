...
def lab(a,s):
    a.text(0.0,1.18,s,transform=a.transAxes,fontsize=FS_LAB,fontweight="bold",
...
a.set_xscale("log"); a.set_xlim(0.25,3.8); a.set_ylim(-0.03,1.36)
...
a.text(TX["breakpoint_DNR"]*0.96,1.11,f"fitted breakpoint ({TX['breakpoint_DNR']:.2f})",
       fontsize=FS_AN,color=ORANGE,ha="right",va="center")
a.text(1.05,1.11,"magnitude crossover",fontsize=FS_AN,color=INK,ha="left",va="center")
a.text(0.265,0.52,"chance",fontsize=FS_AN,color=BROWN,va="bottom")
a.set_xlabel("DNR  (Mahalanobis, log scale)",fontsize=FS_AX)
a.set_ylabel("Open-set AUROC",fontsize=FS_AX)
a.set_title("Twin arrays: the association reappears under unit-to-unit shift",
            fontsize=FS_AX,pad=6)
a.text(0.02,0.99,f"Spearman $\\rho$ = {TW['correlations']['DNR_maha|maha']['rho']:.3f}"
                 f"   (n = {TW['n_splits']})",transform=a.transAxes,
       fontsize=FS_AN,ha="left",va="top",
       bbox=dict(boxstyle="round,pad=0.4",fc="white",ec="#333333",lw=1.0))
a.legend(fontsize=FS_LG-1,loc="lower left",frameon=False,ncol=2,columnspacing=1.0)
...
for off,col,src,lb in [(-h/2,RED,"drift","Drift dataset (n = 95)"),
    ...
    for i,v in enumerate(vs):
        if v<0: a.text(v-lo_[i]-0.045,y[i]+off,f"{v:+.2f}",va="center",ha="right",
                       fontsize=FS_BAR,color=INK)
        else:   a.text(v+hi_[i]+0.045,y[i]+off,f"{v:+.2f}",va="center",ha="left",
                       fontsize=FS_BAR,color=INK)
a.axvline(0,color=INK,lw=1.2,zorder=2)
a.set_yticks(y); a.set_yticklabels([r[0] for r in rows],fontsize=FS_TK-1)
a.set_xlim(-1.25,1.05); a.set_ylim(4.65,-0.75)
a.set_xlabel("Spearman $\\rho$ with open-set AUROC   (cluster bootstrap CI)",fontsize=FS_AX)
a.set_title("The ratio outranks the magnitudes on both datasets",fontsize=FS_AX,pad=6)
a.legend(fontsize=FS_LG,loc="upper right",frameon=False)
...
a.set_ylabel("Spearman $\\rho$ between DNR and AUROC",fontsize=FS_AX)
a.set_xlabel("Novelty score",fontsize=FS_AX)
a.set_title("DNR predicts distance-based scores; confidence moves the other way",
            fontsize=FS_AX,pad=6)
a.legend(fontsize=FS_LG,loc="upper left",frameon=False)
...
a.set_ylim(5.7,-0.7); a.set_xlim(0,1.03)
